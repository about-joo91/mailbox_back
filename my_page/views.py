...
from my_page.serializers import LetterReviewSerializer, LetterUserSerializer
from my_page.services.letter_review_service import delete_letter_review, edit_letter_review, make_letter_review
...
class MyLetterView(APIView):
    ...
    def get(self, request: Request) -> Response:
        ...
        try:
            ...
            return Response(LetterUserSerializer(cur_user).data, status=status.HTTP_204_NO_CONTENT)
        except IndexError:
            return Response(
...
class LetterReviewView(APIView):
    ...
    def get(self, request):
        ...
        letter_review_model = LetterReview.objects.filter(id=letter_review_id)
        return Response(LetterReviewSerializer(letter_review_model).data, status=status.HTTP_200_OK)
    ...
    def post(self, request):
        try:
            ...
            make_letter_review(user=cur_user, letter_id=letter_id, review_data=review_data)
    ...
    def put(self, request):
        try:
            ...
            return Response({"detail": "리뷰 수정에 실패했습니다. 리뷰를 다시 한 번 확인하신 후에 시도해주세요."})
